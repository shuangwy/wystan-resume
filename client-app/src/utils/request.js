...
export default ({
  ...
  accessToken,
  host,
  handleNotOk = handleNotOkFunc,
}) => {
  console.log("api", api, "params", params);
  window.showOffLineError =
    window.showOffLineError === undefined ? false : window.showOffLineError;
  if (window.require && !window.navigator.onLine) {
    console.log("断网了我");
    if (!window.showOffLineError) {
      window.showOffLineError = true;
      message.error("当前网络不可用, 请检查后重试", 3, () => {
        window.showOffLineError = false;
      });
    }
    return null;
  }
